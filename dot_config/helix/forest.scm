...
;; follows conventional tree navigation: collapse an open directory, otherwise
;; move to the directory containing the current entry
(define (forest-back!)
  (define entry (forest-current-entry))
  (when entry
    (define path (car entry))
    (cond
      ;; Search only selects files, so return to that file in the browsable tree
      ;; before applying parent/collapse navigation on the next back action.
      [(forest-searching?)
       (forest-clear-search!)
       (forest-open-ancestors-for-file! path)
       (forest-build-tree!)
       (forest-seek-file! path)]
      ;; #f is the expanded state in *forest-directories*.
      [(and (is-dir? path)
            (hash-contains? *forest-directories* path)
            (not (hash-try-get *forest-directories* path)))
       (forest-toggle-dir! path)]
      [(not (equal? path (helix-find-workspace)))
       (forest-seek-file! (forest-parent-path path))])))
...
(define (forest-command-action! action)
  (cond
    ...
    [(equal? action 'enter) (forest-activate!) event-result/consume]
    [(equal? action 'back) (forest-back!) event-result/consume]
...
(define (forest-handle-event-command state event)
  ...
  (cond
    ...
    [(key-event-left? event) (forest-back!) event-result/consume]
    [(key-event-tab? event)
     (define entry (forest-current-entry))
