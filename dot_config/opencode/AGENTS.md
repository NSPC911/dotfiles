...
- `wezterm cli split-pane`: splits the current pane. same as spawn. supports additional flags like `--left`/`--right`/`--top`/`--bottom` to specify the direction of the split, `--cells`/`--percent` to specify the size of the new pane, and `--cwd` to specify the working directory.
...
- `wezterm cli list --format json`: lists all windows, tabs, and panes in JSON format. useful for getting pane ids.
...
If I ask you "how can xxx be done/fixed", you MUST NOT MAKE CHANGES. Answer with a detailed explanation of how it can be done/fixed, and provide a code snippet if necessary.
Make changes ONLY when I explicitly ask you to do so.
